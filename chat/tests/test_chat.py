...
Covers:
  ...
  - The stream view emits a structured SSE error when the platform key is unset.
  - The guest streaming view requires POST and rejects invalid JSON.

These tests run without hitting the real provider API.
"""
...
from chat.guest import runtime as guest_mod
from chat.guest.runtime import (
    GUEST_MODEL,
    GUEST_PROVIDER,
    fits_token_budget,
    get_guest_user_sync,
...
class GuestSessionEndpointTests(TestCase):
    ...
    def test_create_guest_session_no_auth_required(self):
        ...
        self.assertEqual(body["llm_provider"], GUEST_PROVIDER)
        # Guest mode is one provider and one model; nothing may vary it.
        self.assertEqual(body["llm_model"], GUEST_MODEL)
    ...
    def test_create_guest_session_ignores_requested_model(self):
        """A model named in the request body does not move a guest off the pin."""
        resp = self.client.post(
            "/api/chat/guest/sessions/",
            {"title": "Hi", "llm_provider": "openrouter", "llm_model": "openai/gpt-4o"},
            format="json",
        )
        self.assertEqual(resp.status_code, 201, resp.content)
        session = ChatSession.objects.get(id=resp.json()["id"])
        self.assertEqual(session.llm_provider, GUEST_PROVIDER)
        self.assertEqual(session.llm_model, GUEST_MODEL)
...
class GuestStreamViewTests(TransactionTestCase):
    ...
    async def test_stream_reports_missing_api_key_via_sse(self):
        ...
        self.assertIn("Guest chat is not configured", body)

    async def test_stream_persists_messages_when_provider_returns_text(self):
        """The view persists user + assistant messages when the provider streams content."""
        async def fake_stream(messages, *, model=None, temperature=0.6, max_tokens=4096):
            yield {"type": "content", "content": "Hello, "}
            yield {"type": "content", "content": "world!"}
            yield {"type": "done"}

        with mock.patch("chat.guest.views.stream_guest_chat", side_effect=fake_stream):
            resp = await self.async_client.post(
                self.url,
                data=json.dumps({"content": "hi"}),
                content_type="application/json",
        ...
        self.assertEqual(msgs[1].metadata.get("model"), GUEST_MODEL)

    async def test_stream_repins_a_session_row_naming_another_model(self):
        """A drifted row is corrected, not honoured — the pin is per-request."""
        from asgiref.sync import sync_to_async

        @sync_to_async
        def drift():
            ChatSession.objects.filter(id=self.session_id).update(
                llm_provider="openrouter", llm_model="openai/gpt-4o",
            )
        ...
        async def fake_stream(messages, *, temperature=0.6, max_tokens=4096, **kwargs):
            seen.update(kwargs)
        ...
        # The view never names a model — stream_guest_chat has no such argument.
