...
class Orchestrator:
    @staticmethod
    def normalize_model(raw: str) -> ModelMetadata:
        metadata = ModelMetadata(raw=raw, normalized=raw)
        
        # Try full pattern first
        match = re.search(MODEL_PATTERNS["vendor_family_version"], raw, re.VERBOSE)
        if not match:
            match = re.search(MODEL_PATTERNS["bare_family_version"], raw, re.VERBOSE)
    ...
    @staticmethod
    def analyze_intent(content: str) -> IntentMetadata:
        signals = []
        primary_intent = "chat"
    ...
    @staticmethod
    def orchestrate(raw_model: str, content: str) -> OrchestrationState:
        model_meta = Orchestrator.normalize_model(raw_model)
        intent_meta = Orchestrator.analyze_intent(content)
        
        # Policy Assignment
        policy = PolicyMetadata()
        if "file_write" in intent_meta.signals:
            policy.allow_file_write = True # Grant permission if requested? Or maybe restrict?
            
        # For now, default policy is broad but sanboxed
