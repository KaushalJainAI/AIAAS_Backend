...
workflow_validator — empty, no MCP nodes, disabled server, unavailable server,
                     credential errors, multi-server gather
nodes          — validate_config, execute (success, missing config, cred errors,
                 call error, argument merging)
...
class DrainPoolTests(SimpleTestCase):
    def test_drain_clears_pool(self):
        # Inject a fake entry directly into the module-level pool
        fake_stack = AsyncMock()
        fake_entry = MagicMock()
        fake_entry.stack = fake_stack
        _pool[(999, 1)] = fake_entry

        _run(drain_pool())

        self.assertNotIn((999, 1), _pool)
        fake_stack.aclose.assert_called_once()
...
# ─────────────────────────────────────────────────────────────────────────────
# validate_mcp_nodes (workflow_validator)
# ─────────────────────────────────────────────────────────────────────────────

class WorkflowValidatorTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username="wv_user", password="pw")
        self.server = MCPServer.objects.create(
            name="ValidServer", type="stdio", command="npx", user=None, enabled=True,
        )

    def _workflow(self, *server_ids):
        return {
            "nodes": [
                {"type": "mcp_tool", "config": {"server_id": sid, "tool_name": "go"}}
                for sid in server_ids
            ]
        }

    # Use async_to_sync throughout — see GetServersForUserTests for explanation.

    def test_empty_workflow_returns_no_errors(self):
        errors = async_to_sync(validate_mcp_nodes)({}, self.user)
        self.assertEqual(errors, [])

    def test_no_mcp_nodes_returns_no_errors(self):
        wf = {"nodes": [{"type": "llm"}, {"type": "http"}]}
        errors = async_to_sync(validate_mcp_nodes)(wf, self.user)
        self.assertEqual(errors, [])

    def test_unavailable_server_id_returns_error(self):
        errors = async_to_sync(validate_mcp_nodes)(self._workflow(99999), self.user)
        self.assertEqual(len(errors), 1)
        self.assertIn("99999", errors[0])

    def test_disabled_server_returns_error(self):
        disabled = MCPServer.objects.create(
            name="OffServer", type="stdio", command="npx", user=None, enabled=False,
        )
        errors = async_to_sync(validate_mcp_nodes)(self._workflow(disabled.id), self.user)
        self.assertEqual(len(errors), 1)
        self.assertIn("disabled", errors[0].lower())

    def test_valid_server_with_no_credentials_required_passes(self):
        with patch("mcp_integration.workflow_validator.CredentialInjector.validate",
                   new_callable=AsyncMock, return_value=[]):
            errors = async_to_sync(validate_mcp_nodes)(self._workflow(self.server.id), self.user)
        self.assertEqual(errors, [])

    def test_valid_server_with_credential_error_returns_error(self):
        with patch("mcp_integration.workflow_validator.CredentialInjector.validate",
                   new_callable=AsyncMock, return_value=["Missing GitHub credential"]):
            errors = async_to_sync(validate_mcp_nodes)(self._workflow(self.server.id), self.user)
        self.assertEqual(errors, ["Missing GitHub credential"])

    def test_multiple_servers_all_errors_gathered(self):
        s2 = MCPServer.objects.create(
            name="Server2", type="stdio", command="npx", user=None, enabled=True,
        )
        with patch("mcp_integration.workflow_validator.CredentialInjector.validate",
                   new_callable=AsyncMock, return_value=["err"]):
            errors = async_to_sync(validate_mcp_nodes)(
                self._workflow(self.server.id, s2.id), self.user
            )
        # Both servers return an error — gathered (not fail-fast)
        self.assertEqual(len(errors), 2)

    def test_non_integer_server_id_is_skipped_gracefully(self):
        wf = {"nodes": [{"type": "mcp_tool", "config": {"server_id": "not-a-number", "tool_name": "x"}}]}
        errors = async_to_sync(validate_mcp_nodes)(wf, self.user)
        self.assertEqual(errors, [])

    def test_node_without_server_id_is_skipped(self):
        wf = {"nodes": [{"type": "mcp_tool", "config": {"tool_name": "x"}}]}
        errors = async_to_sync(validate_mcp_nodes)(wf, self.user)
        self.assertEqual(errors, [])

    def test_assert_raises_on_errors(self):
        from .workflow_validator import assert_mcp_nodes_valid, MCPWorkflowValidationError
        with patch("mcp_integration.workflow_validator.CredentialInjector.validate",
                   new_callable=AsyncMock, return_value=["oops"]):
            with self.assertRaises(MCPWorkflowValidationError) as ctx:
                async_to_sync(assert_mcp_nodes_valid)(self._workflow(self.server.id), self.user)
        self.assertIn("oops", str(ctx.exception))


# ─────────────────────────────────────────────────────────────────────────────
# MCPToolNode (nodes.py)
# ─────────────────────────────────────────────────────────────────────────────

class MCPToolNodeValidateConfigTests(SimpleTestCase):
    def setUp(self):
        from .nodes import MCPToolNode
        self.node = MCPToolNode()

    def test_missing_server_id_is_an_error(self):
        errors = self.node.validate_config({"tool_name": "search"})
        self.assertTrue(any("server" in e.lower() for e in errors))

    def test_missing_tool_name_is_an_error(self):
        errors = self.node.validate_config({"server_id": "1"})
        self.assertTrue(any("tool" in e.lower() for e in errors))

    def test_both_present_returns_no_errors(self):
        errors = self.node.validate_config({"server_id": "1", "tool_name": "search"})
        self.assertEqual(errors, [])

    def test_empty_config_returns_two_errors(self):
        errors = self.node.validate_config({})
        self.assertEqual(len(errors), 2)


class MCPToolNodeExecuteTests(SimpleTestCase):
    def setUp(self):
        from .nodes import MCPToolNode
    ...
    def _run_node(self, config, input_data=None):
        return _run(self.node.execute(input_data or {}, config, self.ctx))

    def test_missing_server_id_returns_failure(self):
        result = self._run_node({"tool_name": "search"})
        self.assertFalse(result.success)
        self.assertIn("server_id", result.error)

    def test_missing_tool_name_returns_failure(self):
        result = self._run_node({"server_id": "1"})
        self.assertFalse(result.success)

    def test_credential_missing_returns_failure(self):
        with patch.object(MCPClientManager, "call_tool", new_callable=AsyncMock,
                          side_effect=CredentialMissingError("github", "GitHub")):
            with patch.object(MCPClientManager, "get_server_config", new_callable=AsyncMock,
                              return_value=_fake_server()):
                with patch.object(MCPClientManager, "_resolve_credentials", new_callable=AsyncMock,
                                  return_value=MagicMock(env_vars={}, headers={})):
                    ...
                    with patch.object(MCPClientManager, "_session", mock_session):
                        result = self._run_node({"server_id": "1", "tool_name": "search"})
        self.assertFalse(result.success)
        self.assertIn("github", result.error.lower())

    def test_generic_call_error_returns_failure(self):
        with patch.object(MCPClientManager, "call_tool", new_callable=AsyncMock,
            ...
            with patch.object(MCPClientManager, "get_server_config", new_callable=AsyncMock,
                ...
                with patch.object(MCPClientManager, "_resolve_credentials", new_callable=AsyncMock,
                    ...
                    async def mock_session(self_obj, server, resolved):
                        session = AsyncMock()
                        session.call_tool = AsyncMock(side_effect=RuntimeError("subprocess died"))
                        yield session
    ...
    def test_success_returns_dict_payload(self):
        ok_result = _call_result([_content_block("text", text='{"hits": 3}')])
        ...
        with patch.object(MCPClientManager, "get_server_config", new_callable=AsyncMock,
            ...
            with patch.object(MCPClientManager, "_resolve_credentials", new_callable=AsyncMock,
                ...
                async def mock_session(self_obj, server, resolved):
                    session = AsyncMock()
                    session.call_tool = AsyncMock(return_value=ok_result)
                    yield session

                with patch.object(MCPClientManager, "_session", mock_session):
                    result = self._run_node(
                        {"server_id": "1", "tool_name": "search"},
                        input_data={"q": "test"},
                    )
        self.assertTrue(result.success)

    def test_input_data_merged_over_static_args(self):
        """Dynamic input_data must override static config arguments."""
        captured_args = {}

        ok_result = _call_result([_content_block("text", text="ok")])

        async def capture_call(self_obj, tool_name, arguments):
        ...
        with patch.object(MCPClientManager, "get_server_config", new_callable=AsyncMock,
            ...
            with patch.object(MCPClientManager, "_resolve_credentials", new_callable=AsyncMock,
                              return_value=MagicMock(env_vars={}, headers={})):
                with patch.object(MCPClientManager, "call_tool", capture_call):
                    _run(self.node.execute(
                        {"q": "dynamic"},
                        {"server_id": "1", "tool_name": "search", "arguments": {"q": "static"}},
                        self.ctx,
                    ))

        # input_data value "dynamic" overrides static "static"
        self.assertEqual(captured_args.get("q"), "dynamic")
