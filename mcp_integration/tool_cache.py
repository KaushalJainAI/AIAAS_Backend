...
class MCPToolCache:
    ...
    async def get(server_id: int, user_id: int | None) -> list[dict[str, Any]] | None:
        try:
            return await sync_to_async(cache.get)(_key(server_id, user_id))
    ...
    async def set(server_id: int, user_id: int | None, tools: list[dict[str, Any]]) -> None:
        try:
            await sync_to_async(cache.set)(_key(server_id, user_id), tools, TTL_SECONDS)
        except Exception as e:  # noqa: BLE001
