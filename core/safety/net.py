...
class _ValidatingRedirectHandler(urllib.request.HTTPRedirectHandler):
    ...
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        # Validating only the URL the caller passed is validating the wrong
        # thing: a public URL is free to 302 to 169.254.169.254, and urllib
        # follows it by default. The guard has to apply to whatever we actually
        # end up connecting to, which means every hop.
        assert_url_safe(newurl)
        return super().redirect_request(req, fp, code, msg, headers, newurl)


_FETCH_USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"

#: Ceiling on a single fetched body. Every caller truncates the text it extracts
#: anyway, so the only thing an unbounded `read()` buys is the chance for one
#: hostile response to exhaust the worker's memory before truncation happens.
MAX_FETCH_BYTES = 5 * 1024 * 1024


def fetch_url(url: str, *, timeout: int = 10, max_bytes: int = MAX_FETCH_BYTES) -> bytes:
    """
    Fetch a user- or model-supplied URL with the SSRF guard applied throughout.

    Raises `UnsafeURLError` if the URL — or anything it redirects to — points
    somewhere it must not, and any of urllib's usual errors otherwise. Blocking:
    ...
    with opener.open(request, timeout=timeout) as response:
        # `read(n)` rather than `read()`: a Content-Length header is a claim by
        # the server, not a limit on what it can send.
