...
from core.pagination import decode_cursor, encode_cursor, paginate_keyset
...
class CursorPaginationTests(TestCase):
    def setUp(self):
        ...
        self.workflow = Workflow.objects.create(user=self.user, name="Cursor Flow")
    ...
    def test_keyset_pagination_has_no_duplicates_on_same_timestamp(self):
        now = timezone.now()
        logs = [
            ExecutionLog.objects.create(user=self.user, workflow=self.workflow),
            ExecutionLog.objects.create(user=self.user, workflow=self.workflow),
            ExecutionLog.objects.create(user=self.user, workflow=self.workflow),
        ]
