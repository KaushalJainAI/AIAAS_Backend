"""
The agent configuration contract: `AgentSerializer`, the one way an agent is
validated and saved.

It translates between the frontend's `AgentConfig` (camelCase, flat) and the
model's columns (snake_case, grouped by who reads them). That translation lives
in exactly one place on purpose: the permissions screen an installer sees is
rendered from `tool_grants` / `guardrails` / `requirements`, and the runtime
enforces from those same columns. If a second mapping existed the screen could
promise something the runtime never checks.

Every writer goes through it: the builder and the agents API
(`views/agents.py`), templates and packs (`views/gallery.py`,
`install_packs`), chat's `create_agent` / `update_agent`
(`chat/tools/authoring.py`), publishing, the benchmark installer and revision
restore. That is why it lives here and not in `views/`: a module other apps
import is not a view, and `logs/revisions.py` snapshotting an agent should not
have to import an HTTP layer to do it.

Ids that point at other rows (knowledge bases, skills, connections) are
re-checked against the caller: an agent that could attach someone else's
knowledge base would be a cross-tenant read.
"""
import logging
