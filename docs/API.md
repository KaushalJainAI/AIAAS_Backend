...
| `/api/inference/documents/{id}/office/` | GET/POST | GET: every sheet of an `.xlsx` as raw cells (≤500×40, formulas as source). POST: `{set_cells, append_rows, sheet}` for a workbook, or `{spec}` for a deck/Word file made here | Auth (GET: owner or shared; POST: owner) | Heavy (openpyxl / re-render) | `inference/tests/test_office_edit.py` | `DocumentSerializer` | Document | `inference/office_edit.py`. Workbook edits go through `office/edit.apply` (the `edit_workbook` path, same formula refusals) and drop the stale spec; spec edits re-validate + re-render the real file. Uploaded `.docx/.pptx` (no spec) 400. Stale `expected_updated_at` 412 |
...
| `/api/eval/suites/{id}/world/` | GET | Live + draft world | Auth (owner) | O(1) |  | `EvalWorldSerializer` | EvalWorld | `{live, draft, versions}` — the accepted world sweeps run on, the newest draft awaiting review, every version number |
| `/api/eval/suites/{id}/world/generate/` | POST | Judge-build a world + its cases | Auth (owner) | External (~5 judge calls) |  | `EvalWorldSerializer`, `EvalCaseSerializer` | EvalWorld, EvalCase | `{focus?, cases?≤25}` → 201 `{world, cases, rejected, tokens, cost_usd, model}`. Facts → fixtures → coverage check → cases → blind solve → blind verify → prove (ideal passes, untouched fails) → **draft** world + draft cases on the new version. 400 no agent; 402 judge credential; 502 a pipeline step failed with the reason |
...
| `/api/eval/worlds/{id}/` | GET/DELETE | One world | Auth (owner) | O(1) |  | `EvalWorldSerializer` | EvalWorld | Drafts may be deleted; accepted worlds are history and refuse — regenerate instead |
