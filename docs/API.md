...
[agents.py](../agents/views/agents.py) (agent CRUD and revision restore),
[runs.py](../agents/views/runs.py) (execute, approve, reject, cancel, steer),
[triggers.py](../agents/views/triggers.py) (schedules, and the public webhook
receiver `hooks/<secret>/`),
[hitl.py](../agents/views/hitl.py),
[gallery.py](../agents/views/gallery.py) (Explore, install, publish),
[builder.py](../agents/views/builder.py) (the builder chat),
[capabilities.py](../agents/views/capabilities.py),
[wizard.py](../agents/views/wizard.py), and
[responses.py](../agents/views/responses.py) (OpenAPI-only response shapes).

Validation lives outside the views: `AgentSerializer` is
[agents/config.py](../agents/config.py) and the grant tables are
[agents/grants.py](../agents/grants.py), because other apps import both.

Models: `SubAgent`, `Trigger`, `SchedulerLease`, `HITLRequest`,
`ConversationMessage`, `SharedAgent`. All function-based views. `Workflow` and
its CRUD are gone; the DAG-era tables (`WorkflowVersion`,
`WorkflowTestResult`, `WorkflowCloneHistory`, `TriggerState`) were dropped
2026-08-18 by `orchestrator.0014_drop_dag_era_tables`, and the canvas-era
modules (`executions.py`, `generation.py`, `partial.py`, `versions.py`) with
the workflow canvas they served.

Agents are `SubAgent` rows — see
[docs/AGENT_TEMPLATES.md](AGENT_TEMPLATES.md) §3 for why.
...
| `/api/logs/executions/{id}/` | GET, DELETE | Run detail | Auth (owner) | O(steps + turns + delegation depth) |  | — | ExecutionLog, AgentTurn, AgentStep, SubAgentRevision | Turns nesting their steps, each turn's full reasoning, the revision used, `delegated_by`, and per-step `delegated_runs`. Steps capped at `EXECUTION_NODE_LOG_LIMIT` and turns at `EXECUTION_TURN_LIMIT` (`steps_truncated` / `turns_truncated`); steps with no turn appear under `unattributed_steps`. A malformed UUID is a 404, not a 500. Cost appears at both levels — per turn (priced against the model *that turn* ran on) and per run (the sum) — plus `cost_usd_total` / `cost_source_total` / `delegated_run_count`, which walk the delegation tree one generation at a time, bounded by `MAX_DELEGATION_DEPTH`. `cost_source` is `billed` (the provider's own charge) \| `estimated` (our price table) \| `unpriced` (no price on record) — and **`unpriced` is not zero**: one unpriced turn makes the whole run unpriced, because a total that silently omits a turn is worse than an admitted gap. **DELETE (2026-09-26)** removes one finished run with its turns/steps/trace; live runs 409 (stop first), `caller='eval'` 409 (delete the sweep instead); cost rows kept. Releases file leases and read sets best-effort |
| `/api/logs/executions/bulk-delete/` | POST | Delete many finished runs | Auth | O(n) |  | `BulkDeleteSerializer` (input only) | ExecutionLog | `{ids}` or `{status, older_than_days}` — never neither; atomic, all or nothing. Refuses whole request when any selected row is live or eval-owned. Cost rows kept. Tests: `logs/tests/` |
| `/api/logs/executions/{id}/mark-failed/` | POST | Close one stuck running run as failed | Auth (owner) | O(1) |  | — | ExecutionLog | Only `running` → failed, via the recovery close path (`recovery._fail`) so leases release and `failure_category` is set; anything else 409. For a run stuck after a crash, without waiting for the sweep |
| `/api/activity/live/` | GET | Everything live for the Activity page | Auth | O(n) capped |  | — | ExecutionLog, EvalRun, EvalWorld, ChatSession (read) + in-process registries | One list of live agent runs (eval traffic excluded — a sweep row covers those), detached coding tasks, eval sweeps, generating eval worlds, live chat turns. Capped at `LIVE_LIMIT` (says `truncated`). Chat turns and coding tasks are per-process memory; runs/sweeps/worlds are visible everywhere — stated on the response as `note`. Lives in `logs/` (`logs/activity.py`) as the lowest layer that can read these models; all cross-app reads are function-level (deferred), so no new `.importlinter` exception was needed |
| `/api/activity/recent-files/` | GET | Recent file and content activity | Auth | O(n) capped |  | — | RecentFile, DocumentVersion, CodeChange | Opened files, overwritten versions, workspace edits, newest first, capped at `FILES_LIMIT` (says `truncated`). One feed failing never fails the page |
...
| `/api/inference/recent/` | GET / POST / DELETE | Recently opened files: list (`?app=`, `?types=a,b`, `?limit=` default 20), record an open (`{document_id, app?, view_state?}` → 201), clear all | Auth (owner) | Light | Y (`test_recents.py`) | none (`_row` dict) | inference_recentfile, Document (read) | `inference/recents.py`. One row per (user, document), moved to the top on open. Only files the caller can read and not trashed are listed; a foreign or missing id is the same 404 |
| `/api/inference/recent/<document_id>/` | GET / PATCH / DELETE | Where the user was in one file (`view_state`: page, zoom…), or drop it from the list | Auth (owner) | Light | Y | none | inference_recentfile | `view_state` is flat, ≤16 keys and ≤2 KB serialized, else 400 |
| `/api/inference/app-sessions/<app>/` | GET / PUT | An app's open tabs: restore, or replace wholesale | Auth (owner) | Light | Y | none | inference_appsession | One row per (user, app); `MAX_TABS` = 20, matching the frontend's cap |
...
| `/api/inference/recent/` | GET, POST, DELETE | Recently *opened* files (GET, `?app=`, `?types=`, `?limit=` ≤50); record an open (POST `{document_id, app, view_state?}`, returns the row with the saved `view_state`); clear all (DELETE) | Auth (readable: own or public library) | O(1) write, O(limit) read | `inference/tests/test_recents.py` | — (`DocumentListSerializer` nested) | RecentFile, Document | One row per (user, document); reopening bumps `opened_at`/`open_count`. Listing re-filters trashed, unshared and eval-tree files. 100 rows per user, oldest dropped. Foreign and unknown ids are the same 404. `app` is a shape-checked label, not validated against the catalogue |
| `/api/inference/recent/{document_id}/` | GET, PATCH, DELETE | Read / save where the user is in a file (`view_state`, e.g. PDF page + zoom) without counting an open; forget one file | Auth (readable) | O(1) | `inference/tests/test_recents.py` | — | RecentFile | `view_state` is a flat dict of scalars, ≤16 keys, ≤2 KB, else 400. PATCH creates the row if missing |
| `/api/inference/app-sessions/{app}/` | GET, PUT | An app's open tabs, so a new window or device restores them (PUT `{tabs: [ids], active}`) | Auth (owner of row; ids readable) | O(tabs) | `inference/tests/test_recents.py` | — | AppSession, Document | Stores ids only; names resolved at read. Unreadable/trashed ids dropped on write and read. ≤20 tabs |
...
| `/api/mcp/servers/held-tools/` | GET | Connector tools withheld until the user reviews them, grouped by connection | Auth | O(n) |  | — | MCPToolPin, MCPServer | 2026-09-26, MCP tool pinning (`mcp_integration/pinning.py`): a new tool whose description is addressed to an AI starts `quarantined`, a tool whose name/description/schema digest changed after connection turns `changed`; neither is offered to a model or dispatched (`MCPToolProvider.execute` answers `tool_held`). Per user. One request for the whole Connections page (`HeldToolsPanel`). Tests: `mcp_integration/tests/test_tool_pins.py` |
| `/api/mcp/servers/{pk}/held-tools/approve/` | POST | Trust one held tool in its current form | Auth | O(1) |  | — | MCPToolPin | Body `{tool_name}`. Moves the pending digest to trusted and invalidates `MCPToolCache` so the next turn sees it; 404 when nothing is held under that name; `get_object()` scopes the server to the caller's visible rows. Tests: `test_tool_pins.py` |
...
`agents/grants.py:GRANT_TOOLS` — and this app only serves it and lays the user's
overlay on top.
