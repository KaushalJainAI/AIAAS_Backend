...
- **`get_openai_tool_descriptors(user)`** — OpenAI-format function specs for every tool on every enabled server visible to the user, queried concurrently with a 5 s per-server timeout (`asyncio.wait_for`), so a hung or absent stdio server cannot stall a whole agent turn. A server with a missing credential is **skipped entirely** — an advertised tool that cannot run is worse than one never offered.
